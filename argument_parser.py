...
class ArgumentParser:
    ...
    def parse_and_sync(self, train_dir_param_name="save_dir"):
        opt = self.parse()
        if train_dir_param_name not in opt.__dict__ or opt.__dict__[train_dir_param_name] is None:
            assert False, "Train dir must be given (argument \"%s\")" % train_dir_param_name

        dir = opt.__dict__[train_dir_param_name]
        os.makedirs(dir, exist_ok=True)

        return self.sync(os.path.join(dir, "args.json"))
